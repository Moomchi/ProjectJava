    ...
    <header class="jumbotron">
      ...
      <template>
        <v-container class="grid-list-xl">
          <td v-for="(sauce,index) in sauces" :key="sauce.id">
            <div class="card text-white bg-dark col-lg-4" style="max-width: 15rem;margin: 4px">
              <div class="card-header">{{sauce.sauceName}}</div>
              <img src="../assets/sauce.png" height="200" width="200" />
              <div class="card-body">
                <p class="card-text">
                  {{sauce.price}} лв.
                </p>
                ...
                <div slot="chosen">
                  <b-button variant="secondary" v-on:click="saveProduct(quantities[index],sauce.id)">Добави</b-button>
                </div>
              </div>
            </div>
          </td>
...
export default {
  ...
  data () {
    return {
      ...
      fields: [
        { key: 'sauceName', label: 'Търси по име:' }
      ],
      currentPage: '',
      perPage: 5,
      rows: ''
