    ...
    <header class="jumbotron">
      ...
      <template>
        <v-container class="grid-list-xl">
          <td v-for="(dessert,index) in desserts" :key="dessert.id">
            <div class="card text-white bg-dark col-lg-4" style="max-width: 15rem;margin: 4px">
              <div class="card-header">{{dessert.dessertName}}</div>
              <img src="../assets/dessert.png" height="200" width="200" />
              <div class="card-body">
                <p class="card-text">
                  {{dessert.price}} лв.
                </p>
                ...
                <div slot="chosen">
                  <b-button variant="secondary" v-on:click="saveProduct(quantities[index],dessert.id)">Добави</b-button>
                </div>
              </div>
            </div>
          </td>
...
export default {
  ...
  data () {
    return {
      ...
      fields: [
        { key: 'dessertName', label: 'Търси по име:' }
      ],
      currentPage: '',
      perPage: 5,
      rows: ''
